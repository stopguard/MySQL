...
/*
 ===================================
 Дополнения в процессе выполнения дз
 ===================================
 */
-- Таблица постов пользователей, что бы было куда ссылаться лайкам на посты.
CREATE TABLE posts (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT "Идентификатор строки",
  user_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя, который загрузил сообщение",
  media_id INT UNSIGNED COMMENT "Ссылка на прикреплённое медиа",
...
CREATE TABLE likes (
  ...
  media_id INT UNSIGNED COMMENT "Ссылка на лайкнутое медиа",
  post_id INT UNSIGNED COMMENT "Ссылка на лайкнутый пост",
  liked_user_id INT UNSIGNED COMMENT "Ссылка на лайкнутого пользователя",
  FOREIGN KEY (user_id) REFERENCES users(id),
  FOREIGN KEY (media_id) REFERENCES media(id),
